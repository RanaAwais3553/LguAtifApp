...
export const deleteContactUser = (contactUserIndex) => {
  return async (dispatch, getState) => {
    ...
    await fetch(
      `https://progccmobileapp-default-rtdb.firebaseio.com/contact/${contactUserIndex}.json?auth=${token}`,
      {
        method: 'DELETE',
      }
    )
  ...
  return async (dispatch, getState) => {
    ...
    try {
      const response = await fetch(
        `https://progccmobileapp-default-rtdb.firebaseio.com/contact.json?auth=${token}`
      )
